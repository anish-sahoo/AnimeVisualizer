...
collection = db.anime_details_ext
...
# Convert 'members_count', 'popularity', 'ranked', and 'favorited_count' to numeric and handle missing values
df['members_count'] = pd.to_numeric(df['members_count'].str.replace(',', ''), errors='coerce').fillna(0)
df['popularity'] = pd.to_numeric(df['popularity'].str.replace('#', ''), errors='coerce').fillna(0)
df['ranked'] = pd.to_numeric(df['ranked'].str.replace('#', '').str.replace(',', ''), errors='coerce').fillna(0)
df['favorited_count'] = pd.to_numeric(df['favorited_count'].str.replace(',', ''), errors='coerce').fillna(0)

# Standardize the numerical features (optional)
members_popularity_ranked_favorited_scaled = scaler.fit_transform(df[['members_count', 'popularity', 'ranked', 'favorited_count']])
members_popularity_ranked_favorited_scaled_df = pd.DataFrame(members_popularity_ranked_favorited_scaled, columns=['members_count_scaled', 'popularity_scaled', 'ranked_scaled', 'favorited_count_scaled'])

# Concatenate with other features
# combined_embeddings = pd.concat([
#     combined_embeddings,
#     members_popularity_ranked_favorited_scaled_df
# ], axis=1)


# Select the columns to store
columns_to_store = ['title', 'english_title', 'japanese_title', 'score', 'genres', 'themes', 'demographic', 'synopsis', 'url', 'studio', 'type', 'episode_count', 'year_first_aired', 'favorited_count', 'members_count', 'popularity', 'ranked']
